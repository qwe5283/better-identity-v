...
public partial class PickerWindow : FluentWindow
{
    ...
    {
        return window is
            { Name: "第五人格", ProcessName: "dwrg" } or
            { Name: "MuMu模拟器12", ProcessName: "MuMuPlayer" };
    }
    
    private static bool AskIsThisIdentityV(CapturableWindow window)
    {
        var res = Wpf.Ui.Violeta.Controls.MessageBox.Question(
            $"""
             这看起来不像是第五人格，确定要选择这个窗口吗？
    ...
    private void WindowsOnMouseDoubleClick(object sender, MouseButtonEventArgs e)
    {
        ...
        // 如果不是游戏窗口，询问用户是否确认
        if (!_captureTest && !IsGameWindow(selectedWindow))
        {
            if (!AskIsThisIdentityV(selectedWindow))
            {
                return;
            }
        }
